...
void Mushmom::HandleJumpProcess()
{
    ...
    if (jumpingState == JUMPING_STATE::JUMPING_UP)
    {
        ...
        worldPos.x = worldPos.x + velocity * cos(PI / 2.0f) * jumpTimer;
        worldPos.y = priorPosY - (velocity * sin(PI / 2.0f) * jumpTimer - 0.5f * GA * jumpTimer * jumpTimer);// 점프타이머(-0.5*GA*점프타이머 + velocity * sin(PI/2.0f))
    ...
    else if (jumpingState == JUMPING_STATE::JUMPING_DOWN)
    {
        ...
        if (jumpTimer < velocity * sin(PI / 2.0f) / (0.5f * GA) / 2.0f) {//낙하시간이 아니라 이륙시간대이면
            ...
            priorPosY = worldPos.y + (velocity * sin(PI / 2.0f) * jumpTimer - 0.5f * GA * jumpTimer * jumpTimer);
        }
        jumpTimer += TimerManager::GetSingleton()->GetElapsedTime() * 10;
        worldPos.x = worldPos.x + velocity * cos(PI / 2.0f) * jumpTimer;
        worldPos.y = priorPosY - (velocity * sin(PI / 2.0f) * jumpTimer - 0.5f * GA * jumpTimer * jumpTimer);
    }
...
void Mushmom::HandleWalkingState()
{
    Move();
    if (timer >= 3.0f) {
        if (player != NULL && CalcUtil::GetSingleton()->GetDistance(player->GetWorldPos(), worldPos) <= attackRange)
        {
            state = UNIT_STATE::ATTACK_STATE;
            if (player->GetWorldPos().x < worldPos.x)
                moveDirection = MOVE_DIRECTION::MOVE_LEFT;
            else
...
void Mushmom::HandleAttackState()
{
    ...
    if (jumpingState == JUMPING_STATE::END_OF_JUMPING_STATE && frame.x == 7)//처음 점프를 했을 때
    {
        ...
        SetRect(&attackBox, worldPos.x - attackRange, worldPos.y + GetCenterToBottom() - 50, worldPos.x + attackRange, worldPos.y + GetCenterToBottom() + 50);
    }
